                        ...
                        <div class="mb-3 row">
                            ...
                            <div class="col-md-6">
                                <select class="form-control @error('type') is-invalid @enderror" id="type"
                                    name="type" onchange="filterVolunteers()">
                                    ...
                                    @foreach ($components as $component)
                                        <div>
                                            ...
                                            <label for="component-{{ $component->id }}"
                                                class="form-check-label">{{ $component->titleComponente }}</label>
                                        </div>
                                    @endforeach
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <select class="form-control @error('volunteer_id') is-invalid @enderror" id="volunteer_id"
                                    ...
                                    @foreach ($volunteers as $volunteer)
                                        @php
                                            $permissions = $volunteer->roles->flatMap->permissions
                                                ->pluck('name')
                                                ->unique();
        ...
        function filterVolunteers() {
            ...
            for (const volunteer of volunteers) {
                ...
                if (isVisible) {
                    ...
                    roles.forEach(role => {
                        if ((type === 'create-blog' || type === 'create-tour') && role.includes(
                            'Volunteer senior')) {
                            volunteersTypes.add('Volunteer senior');
                        } else if ((type === 'create-donation' || type === 'create-component') && role.includes(
                                'Volunteer junior')) {
            ...
            document.getElementById('volunteerTypeRequired').textContent = volunteersTypes.size > 0 ? Array.from(
                volunteersTypes).join(', ') : 'No hay voluntarios disponibles para este tipo de tarea';
        }
