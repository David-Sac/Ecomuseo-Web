        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('tasks.update', $task->id) }}" method="post">
                    ...
                    <div class="mb-3 row">
                        ...
                        <div class="col-md-6">
                            <select class="form-control @error('type') is-invalid @enderror" id="type" name="type" onchange="filterVolunteers()">
                                ...
                                <option value="create-component" {{ $task->type == 'component' ? 'selected' : '' }}>Mantenimiento de componentes</option>
                            </select>
                            @error('type')
                                ...
                                @foreach ($components as $component)
                                    <div>
                                        <input type="checkbox" class="form-check-input" id="component-{{ $component->id }}" name="components[]" value="{{ $component->id }}"
                                               @if(in_array($component->id, $selectedComponents)) checked @endif>
                        ...
                        <div class="col-md-6">
                            <select class="form-control @error('volunteer_id') is-invalid @enderror" id="volunteer_id" name="volunteer_id">
                                ...
                                @foreach ($volunteers as $volunteer)
                                    ...
                                    @endphp
                                    <option value="{{ $volunteer->id }}" data-permissions="{{ $permissions->join(',') }}" data-roles="{{ $roles->join(',') }}" {{ $volunteer->id == $volunteerId ? 'selected' : '' }}>
                                        {{ $volunteer->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="mb-3 row">
                        <input type="submit" class="col-md-3 offset-md-5 btn btn-primary" value="Actualizar tarea">
                    </div>
                </form>
            </div>
        </div>
...
// Ejecutar filtro al cargar la página para mostrar el estado correcto del checklist de componentes
